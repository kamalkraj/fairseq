...
from fairseq.modules import DebertaV2Layer, StableDropout, build_relative_position
...
class ConvLayer(nn.Module):
    ...
    def forward(self, hidden_states, residual_states, input_mask):
        ...
        out = utils.get_activation_fn(self.conv_act)(self.dropout(out))
...
class DebertaV2Model(nn.Module):
    def __init__(self, config, share_embed_tokens=None, share_emb_layer_norm=None):
        super().__init__()

        self.embeddings = DebertaV2Embeddings(config)
        self.encoder = DebertaV2Encoder(config)
        self.config = config
        if share_embed_tokens != None:
            self.set_input_embeddings(share_embed_tokens)
    ...
    def get_input_embeddings_layer_norm(self):
        return self.embeddings.LayerNorm

    def set_input_embeddings(self, new_embeddings):
        self.embeddings.word_embeddings = new_embeddings

    def set_input_embeddings_layer_norm(self, new_layer_norm):
        self.embeddings.LayerNorm = new_layer_norm
